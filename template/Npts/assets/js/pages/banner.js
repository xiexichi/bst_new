	...
	if(data.errno==0){
		...
		for(var i=0;i<length;i++){
				...
				if(data.edit_auth||data.del_auth){
					...
					b+='<td>';
					if(data.edit_auth){
						b+='<a href="'+head_url+'banner/edit.html?id='+data.data[i]['id']+'" class=" bk-fg-darken"><small>编辑</small> <i class="fa  fa-pencil"></i></a>';
						b+='<div class="radio-custom radio-inline"></div>';
					}
					if(data.del_auth){
						b+='<a href="adminapi.html?url=admin_banner_del&id='+data.data[i]['id']+'" class=" bk-fg-darken ajax-get" callback="del_callback"><small>删除</small> <i class="fa  fa-ban"></i></a>';
					}
					b+='</td>';
				}
				b+='</tr>';
