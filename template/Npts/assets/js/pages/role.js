...
function in_callback(data){
	...
	console.log(data);
	if(data.errno==0){
		$('#role_list').removeClass('hide');
		$('#role_button').removeClass('hide');
		...
		for(var i=0;i<length;i++){
				b+='<tr>';
				b+='<td>'+(i+1)+'</td>';
				b+='<td>'+data.data[i]['title']+'</td>';
				b+='<td>'+data.data[i]['description']+'</td>';
				b+='<td>'+data.data[i]['create_time']+'</td>';
				if(data.edit_auth||data.del_auth||data.auth_auth||data.allot_auth){
					$('#edit_auth').removeClass('hide');
					b+='<td>';
					if(data.edit_auth){
						b+='<a href="'+head_url+'role/edit.html?id='+data.data[i]['id']+'" class=" bk-fg-darken"><small>编辑</small> <i class="fa  fa-pencil"></i></a>';
						b+='<div class="radio-custom radio-inline"></div>';
					}
					if(data.allot_auth){
						b+='<a href="'+head_url+'role/allot.html?id='+data.data[i]['id']+'" class=" bk-fg-darken"><small>角色分配</small> <i class="fa  fa-user"></i></a>';
						b+='<div class="radio-custom radio-inline"></div>';
					}
					if(data.auth_auth){
						b+='<a href="'+head_url+'auth.html?group='+data.data[i]['id']+'" class=" bk-fg-darken"><small>权限分配</small> <i class="fa  fa-wrench"></i></a>';
						b+='<div class="radio-custom radio-inline"></div>';
					}
					if(data.del_auth){
						b+='<a href="adminapi.html?url=admin_role_del&id='+data.data[i]['id']+'" class=" bk-fg-darken ajax-get" callback="del_callback"><small>删除</small> <i class="fa  fa-ban"></i></a>';
					}
					b+='</td>';
				}
				b+='</tr>';
