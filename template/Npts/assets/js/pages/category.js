...
function in_callback(data){
	...
	if(data.errno==0){
		...
		$('#category_button').removeClass('hide');
		...
		for(var i=0;i<length;i++){
				...
				b+='<td>'+data.data[i]['sort']+'</td>';
				b+='<td>'+data.data[i]['create_time']+'</td>';
				if(data.edit_auth||data.del_auth){
					$('#edit_auth').removeClass('hide');
					b+='<td>';
					if(data.edit_auth){
						b+='<a href="'+head_url+'category/edit.html?id='+data.data[i]['id']+'" class=" bk-fg-darken"><small>编辑</small> <i class="fa  fa-pencil"></i></a>';
						b+='<div class="radio-custom radio-inline"></div>';
					}
					if(data.del_auth){
						b+='<a href="adminapi.html?url=admin_category_del&id='+data.data[i]['id']+'" class=" bk-fg-darken ajax-get" callback="del_callback"><small>删除</small> <i class="fa  fa-ban"></i></a>';
					}
					b+='</td>';
				}
